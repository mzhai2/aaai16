function [clusterIndices,purity,plotfig,datafig] = plotKmedoids(mappedX2,foundWords,foundCategories,kernelMethod,vectors,numCategories,vocab2cat,distance,subscript,showWindows,visible)

opts = statset('Display','final','UseParallel',true);
[clusterIndices, medoids] = kmedoids(vectors,numCategories,'Distance',distance,'Replicates',10,'Options',opts);
global ind;
...
if (showWindows == 1 || showWindows==3)
    ...
    title(strcat('k-medoids using ',distance,' distance ', kernelMethod));
    if subscript
